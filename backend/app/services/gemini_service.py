...
- Revenue: ₹{financial_summary.get('revenue', 0):,.0f}
- Net Income: ₹{financial_summary.get('net_income', 0):,.0f}
- Total Assets: ₹{financial_summary.get('total_assets', 0):,.0f}
- Operating Cash Flow: ₹{financial_summary.get('operating_cash_flow', 0):,.0f}
...
- Working Capital: ₹{financial_ratios.get('working_capital', 0):,.0f}
...
- Additional Debt Capacity: ₹{ml_prediction.get('debt_capacity', 0):,.0f}
...
The final decision is always made by a human loan officer.
"""
...
def _format_loans(loans: list[dict]) -> str:
    ...
    for l in loans:
        lines.append(
            f"  - {l.get('loan_type', 'Loan').replace('_', ' ').title()}: "
            f"₹{l.get('outstanding_balance', 0):,.0f} outstanding, "
            f"₹{l.get('monthly_emi', 0):,.0f}/month EMI"
        )
...
def _fallback_summary(name: str, fin: dict) -> str:
    ...
    return (
        f"{name} operates with annual revenue of approximately ₹{revenue:,.0f} "
        f"and net income of ₹{net:,.0f}. The farm shows steady operational performance "
...
def _fallback_loans(loans: list[dict]) -> str:
    ...
    return (
        ...
        f"₹{total:,.0f} and a combined monthly EMI of ₹{total_emi:,.0f}. "
...
def _fallback_risks(risks: dict) -> str:
    ...
    return (
        f"Weather conditions show {'favorable' if weather.get('drought_index', 0) < 0.3 else 'moderate'} "
        f"rainfall with {'low' if weather.get('flood_risk') == 'low' else 'some'} flood risk. "
        f"Commodity prices have changed {commodity.get('price_change_pct', 0):+.1f}% recently. "
        f"Input costs remain at typical levels."
    )
...
def _fallback_recommendation(pred: dict) -> str:
    ...
    if risk == "low":
        return (
            ...
            f"Estimated additional debt capacity of ₹{capacity:,.0f}. "
...
def _fallback_evidence(ratios: dict, pred: dict) -> str:
    return (
        ...
        f"• Working capital of ₹{ratios.get('working_capital', 0):,.0f} — "
