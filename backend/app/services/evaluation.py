...
def compute_threshold_curve(results: list[dict]) -> dict:
    """
    Compute metrics at multiple thresholds to show the precision-recall trade-off.
    Allows the user to understand the business impact of threshold choice.
    """
    ...
    for t in thresholds:
        tp = fp = tn = fn = 0
        for r in results:
            pred_high = r["predicted_risk_score"] >= t
            actual_high = r["actual_high_risk"]
        ...
        manual_reviews = tp + fp  # farmers flagged as high risk

        interpretation = ""
        if t <= 0.35:
            interpretation = "Growth-focused: higher recall, more approvals, some risky loans may pass"
        elif t <= 0.45:
            interpretation = "Balanced-growth: good recall, moderate review volume"
        elif t <= 0.55:
            interpretation = "Standard: balanced precision and recall"
        elif t <= 0.65:
            interpretation = "Conservative: higher precision, fewer false approvals"
        else:
            interpretation = "Risk-averse: maximum precision, most applications sent for review"
    ...
    return {
        ...
        "note": (
            "Lower threshold = higher recall (catch more risky farmers) "
            "but more false positives (unnecessary manual reviews). "
            "Higher threshold = higher precision (fewer false alarms) "
            "but more false negatives (missed defaults — more expensive)."
        ),
    }
