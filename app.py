...
PIPELINE = [
    {"id":"AG-2026-0001","name":"Erik Johansson","region":"Skane","district":"Lund","status":"Draft","score":87,"dscr":1.32,"ha":85,"crop":"Hostvete","years":18,"uc":720,"insurance":True},
    {"id":"AG-2026-0002","name":"Anna Nilsson","region":"Ostergotland","district":"Linkoping","status":"Draft","score":82,"dscr":1.45,"ha":120,"crop":"Varvete","years":22,"uc":780,"insurance":True},
    ...
    {"id":"AG-2026-0005","name":"Johan Karlsson","region":"Skane","district":"Ystad","status":"Draft","score":91,"dscr":1.85,"ha":150,"crop":"Hostvete","years":25,"uc":820,"insurance":True},
    ...
    {"id":"AG-2026-0008","name":"Sofia Berg","region":"Varmland","district":"Karlstad","status":"In Review","score":72,"dscr":1.18,"ha":70,"crop":"Varkorn","years":12,"uc":690,"insurance":True},
]
...
def product_banner(pred, role_label):
    ...
    pl_status = st.session_state.pipeline_overrides.get(cf["name"], cf.get("status", "Draft"))
    ...
    elif pl_status in ("In Review",):
        status_color = "#fbbf24"; status_text = pl_status.upper()
    elif pl_status in ("Draft","Missing Docs","Flagged"):
        status_color = "#34d399" if pl_status == "Draft" else "#fbbf24" if pl_status == "Missing Docs" else "#f87171"
...
def register_farmer():
    ...
    with c3:
        status = st.selectbox("Initial Pipeline Status",
                             ["Draft","Missing Docs","Flagged","In Review"],
                             key="reg_status")
...
def analyst_pipeline():
    ...
    pl = get_pipeline()
    ready = sum(1 for p in pl if p["status"]=="Draft")
    ...
    in_progress = sum(1 for p in pl if p["status"]=="In Review")
    ...
    if "analyst_filter" not in st.session_state: st.session_state.analyst_filter = "Draft"
    ...
    for p in filtered:
        status = p['status']
        badge = "g" if status in ("Draft","Submitted","Approved","Sent to Bank") else "y" if status in ("Missing Docs","In Review","Approved w/ Cond") else "r" if status in ("Flagged","Rejected") else "g"
...
def bank_pipeline():
    ...
    sent_to_bank = sum(1 for p in pl if p["status"]=="Sent to Bank")
    approved = sum(1 for p in pl if p["status"] in ("Approved","Approved w/ Cond"))
    in_progress = sum(1 for p in pl if p["status"] in ("In Review","Missing Docs","Flagged"))
    rejected = sum(1 for p in pl if p["status"]=="Rejected")
    ...
    if active_filter == "Awaiting":
        filtered = [p for p in pl if p["status"] in ("Draft","Submitted","Sent to Bank")]
    ...
    for p in filtered:
        c1,c2,c3,c4,c5 = st.columns([2.5,1.5,1,1,1])
        with c1: st.markdown(f"**{p['name']}**  \n<small style='color:#667085'>{p.get('district','')}, {p['region']}</small>",unsafe_allow_html=True)
        with c2: st.markdown(f"<span class='badge badge-{'g' if p['status'] in ('Draft','Submitted','Approved','Sent to Bank') else 'y' if p['status'] in ('Missing Docs','In Review','Approved w/ Cond') else 'r'}'>{p['status']}</span>",unsafe_allow_html=True)
        with c3: st.markdown(f"<span style='color:#e8ecf1;font-size:0.85rem;font-weight:600;'>{p['score']}%</span>",unsafe_allow_html=True)
        with c4: st.markdown(f"<span style='color:#667085;font-size:0.8rem;'>DSCR {p['dscr']}</span>",unsafe_allow_html=True)
        with c5:
